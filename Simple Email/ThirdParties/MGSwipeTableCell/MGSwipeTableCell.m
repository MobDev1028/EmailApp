...
{
    if (_fromLeft == YES) {
        _firstButton = [_buttons objectAtIndex: _buttons.count - 1];
    }
    else
    {
        _firstButton = [_buttons objectAtIndex: 0];
    }
}
...
@implementation MGSwipeTableCell
{
    ...
    __weak MGSwipeButtonsView * _firstExpansion;
...
- (void)setSwipeOffset:(CGFloat) newOffset;
{
    ...
    if (!activeButtons || offset == 0) {
        ...
        CGFloat swipeThreshold = 1.75;//activeSettings.threshold;
...
-(void) setSwipeOffset:(CGFloat)offset animation: (MGSwipeAnimation *) animation completion:(void(^)(BOOL finished)) completion
{
    ...
    _animationData.duration = animation.duration;//0.8f;
...
-(void) panHandler: (UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (expansion) {
            ...
            if(animation == settings.stretchAnimation || animation == settings.showAnimation)
            {
                MGSwipeExpansionSettings * expSettings = _swipeOffset > 0 ? _leftExpansion : _rightExpansion;
                [self setSwipeOffset:_targetOffset animation:expSettings.triggerAnimation completion:^(BOOL finished){
                    if (!finished || self.hidden) {
                    ...
                    BOOL autoHide = [_firstExpansion handleClick:[_firstExpansion getFirstButton] fromExpansion:YES];
